 ...
 * policy (write to a temp file in the same directory, fsync, rename,
 * fsync the parent) and the path jail (percent-decode, then reject any
 * "..", NUL, or control byte before a single syscall runs) sit in one
 * place. The HTTP layer never builds a filesystem path itself; it calls
 ...
 * `content_length` bytes are stored, via temp -> fsync -> rename ->
 * fsync(dir). Parent directories are created. 0 / -1 (errno). */

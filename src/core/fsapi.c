 ...
 * final rename is always within one filesystem and therefore atomic,
 * wherever the target lands under the share.
 * ==================================================================== */
...
/* Percent-decode `in` into `out`. Rejects a decoded NUL and malformed
 * escapes (errno=EINVAL); ENAMETOOLONG if `out` would overflow. */
                       ...
                       long long content_length)
{
    ...
    if (close(fd) != 0) { fd = -1; goto fail_noclose; }
...
int fsapi_copy(const char *src, const char *dst)
{
    ...
    if (close(out) != 0) { out = -1; goto fail_noclose; }
